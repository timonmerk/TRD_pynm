...
def plot_performance_fft_sharpwave(PATH_RESULTS: str):
    df_all = pd.read_csv(
        os.path.join(PATH_RESULTS, "all_fatures_lm.csv")
    )  # csv unfortunately does not save the coef array correctly


def plot_performances_across_methods(PATH_RESULTS: str):
    df_all = pd.read_csv(
        os.path.join(PATH_RESULTS, "all_fatures_lm.csv")
    )  # csv unfortunately does not save the coef array correctly

    df_wf_fft = pd.read_csv(
        os.path.join(PATH_RESULTS, "df_features_WF_fft_LM.csv")
    )

    df_wf_all = pd.read_csv(
        os.path.join(PATH_RESULTS, "df_features_WF_all_LM.csv")
    )

    df_pca = pd.read_csv(os.path.join(PATH_RESULTS, "all_fatures_pca.csv"))
    df_pca_xgb = pd.read_csv(
        os.path.join(PATH_RESULTS, "all_fatures_pca_xgb.csv")
    )
    df_cca = pd.read_csv(os.path.join(PATH_RESULTS, "all_fatures_cca.csv"))
    df_cca_xgb = pd.read_csv(
        os.path.join(PATH_RESULTS, "all_fatures_cca_xgb.csv")
    )
    df_all_xgb = pd.read_csv(os.path.join(PATH_RESULTS, "all_fatures_xgb.csv"))
    df_mrmr = pd.read_csv(os.path.join(PATH_RESULTS, "mrmr_fatures.csv"))
    df_mrmr_20 = pd.read_csv(os.path.join(PATH_RESULTS, "mrmr_fatures_20.csv"))
    df_mrmr_xgb = pd.read_csv(
        os.path.join(PATH_RESULTS, "mrmr_features_xgb.csv")
    )
    df_fft = pd.read_csv(os.path.join(PATH_RESULTS, "fft_fatures.csv"))
    df_fft_xgb = pd.read_csv(os.path.join(PATH_RESULTS, "fft_features_xgb.csv"))
    ...
    df_wf_fft["features_used"] = "lm_wf_fft"
    df_wf_all["features_used"] = "lm_wf_all"
...
def plot_channels_selected_mrmr():
    ...
    val_cnt = pd.Series(
        [f[8:] for f in used_features]
    ).value_counts()  # 8 to cut off channel name
...
def plot_lm_channel_coefficients_three_classes():
    # plot mean coefficients
    mean_coefficients_LM = np.stack(
        pd.concat(pd_data).query("all_combined == 0")["coef"]
    ).mean(axis=0)
    ...
    for f_idx in range(len(feature_names)):
        for idx, stim in enumerate([(2, "PLS"), (3, "UNPLS")]):  # (1, "NTR"),
            df_plt_coef = df_plt_coef.append(
                {
                    "val": mean_coefficients_LM[idx, f_idx],
    ...
    sb.barplot(
        x="f_name", y="val", hue="class", data=df_plt_coef, palette="viridis"
    )
    plt.xticks(rotation=90)
    plt.savefig(
        r"C:\Users\ICN_admin\Documents\TRD Analysis\30_05\result_figures\LM_coeff_multiclass.png",
        bbox_inches="tight",
    )
...
OMIT_REST_CLASS = False
GET_FEATURE_EPOCHS = False
GET_AVG_LABEL = True
ONLY_PLS_UNPLS = True
...
for sub in subjects:
    ...
    if OMIT_REST_CLASS:
        ...
        analyzer.label = np.array(
            analyzer.feature_arr.loc[
                analyzer.feature_arr["ALL"] != 0
                and analyzer.feature_arr["ALL"] != 1,
                :,
            ].iloc[:, LABEL]
        )
    ...
    # analyzer.feature_arr = analyzer.feature_arr.iloc[:, :-5]
    ...
    if GET_AVG_LABEL is True:
        ...
        for label_ in [2, 3]:  # 1,
            ...
            y_te_pr = np.concatenate(
                analyzer.decoder.all_ch_results["y_test_pr"]
            )
            y_te[np.where(y_te != label_)[0]] = 0
            y_te[np.where(y_te == label_)[0]] = 1
...
reducer = umap.UMAP()
embedding = reducer.fit_transform(pd.concat(data_all))
plt.scatter(embedding[:, 0], embedding[:, 1], c=np.concatenate(label))
plt.title("UMAP projection all subject data")
plt.savefig(
    r"C:\Users\ICN_admin\Documents\TRD Analysis\30_05\result_figures\UMAP_AllSub.png",
)
